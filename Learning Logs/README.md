...
| `04-server-bootstrap-and-analytics-rollup.md` | `server.js` rewritten from stub to real bootstrap (middleware stack, `/` + `/health`, connect-before-listen, graceful shutdown, process safety nets); the Express error middleware; and `endpoint_metrics`, the PostgreSQL pre-aggregation rollup table | Committed in `b454598`, refined in `04c104e` |
| `05-getting-it-running-and-containerized.md` | The startup-blocking bug fixes from Phases 01/04 (`winston.format.combine`, the `MongoConnection` singleton, `import cors`, the `init.postgres.sql` rename); getting Postgres to actually accept connections; and full dockerisation - both Dockerfiles, `.dockerignore`, and `api-app` + gated `consumer` services in compose | Complete up to commit `40eb9b7` |
